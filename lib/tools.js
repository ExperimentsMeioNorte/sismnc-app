...
// remove o bug do cordova facebook, sempre no primeiro acesso o cordova no funciona, entao tem que reiniciar o app pra funcionar
/*Meteor.removeBugCordovaFacebook = function(){
  alert(localStorage.getItem('Meteor.getupLogin'));

  if(localStorage.getItem('Meteor.getupLogin') !== 'configurado'){
    IonLoading.show({
      customTemplate: '<b style="color:#000;">Configurando o aplicativo...</b>'
    });

    Meteor.setTimeout(function () {
      localStorage.setItem('Meteor.getupLogin', 'configurado');
      IonLoading.hide();
