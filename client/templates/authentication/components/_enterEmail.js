Template._enterEmail.events({
    'touchstart .btn-reset-pass': function () {
        document.querySelector('body').classList.add('show-reset-password');
    },

    'touchstart .close-reset-password': function () {
        document.querySelector('body').classList.add('show-reset-password');
    },

    'touchstart .bg-password': function (event, tmp) {
        IonLoading.show({
        ...
        if(email !== '' && password !== ''){
            var userId = User.findOne({
                email:              email,
                password:           CryptoJS.MD5(email + password).toString(),
                facebook_id:        null,
                google_id:          null
            });
            ...
            // verifica se o usuario foi bloqueado
                ...
                IonLoading.hide();
                toastr.info(
                    "Rum, Você não tem autorização para acessar, fez danação em.",
                    '',
                    {
                        "positionClass": "toast-top-center",
                        "tapToDismiss": true,
